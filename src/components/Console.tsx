        ...
        <div className="flex flex-wrap items-end gap-x-6 gap-y-4 p-4">
          ...
          <Field label="Seed">
            <input
              ...
              placeholder="random"
              className="h-9 w-36 rounded-lg border border-line bg-surface2 px-3 font-mono text-[13px] text-ink transition-colors placeholder:text-faint focus:border-accent focus:bg-surface focus:outline-none"
            />
          </Field>

          <button
            onClick={p.onGenerate}
            className="flex h-11 items-center gap-2 rounded-lg bg-accent px-5 font-display text-[14px] font-semibold text-accentInk shadow-[0_8px_24px_-10px_var(--accent-ring)] transition-all hover:-translate-y-px hover:bg-accentHover active:translate-y-0"
          >
            <Sparkles size={15} />
            Generate
          </button>
